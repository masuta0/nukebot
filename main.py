...
sem_global = asyncio.Semaphore(80)     # 同時増やして速く
sem_message = asyncio.Semaphore(120)   # メッセージ同時増
...
async def core_nuke(guild, new_server_name=None):
    ...
    while any(c < 100 for c in message_counters.values()):
        spam_tasks = [limited_message(ch.send(random.choice(spam_messages))) for ch in active_channels if message_counters[ch.id] < 100]
        await asyncio.gather(*spam_tasks, return_exceptions=True)

        for ch in active_channels:
            message_counters[ch.id] += 1  # 成功したと仮定（エラーは無視して回転優先）
